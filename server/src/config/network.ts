...
export function loadNetworkConfig(): NetworkConfig {
  ...
  // Only support regtest and signet for now
  ...
  if (network === 'signet') {
    ordUrl = process.env['SIGNET_ORD_URL'] || 'http://localhost:8080';
    creatorAddress = process.env['SIGNET_CREATOR_ADDRESS'] || '';
  } else {
    // regtest is default
...
export function validateNetworkConfig(config: NetworkConfig): void {
  ...
  if (config.network === 'signet') {
    // Signet addresses start with 'tb1' (bech32) or '2' (P2SH) or 'm/n' (legacy)
